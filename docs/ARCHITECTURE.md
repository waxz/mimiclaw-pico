...
> ESP32-C3 AI Agent firmware — C/FreeRTOS implementation running on bare metal (no Linux).
...
| TLS connections x2 (Telegram + Claude) | Internal SRAM      | ~120 KB  |
| JSON parse buffers                 | Internal SRAM          | ~32 KB   |
| Session history cache              | Internal SRAM          | ~32 KB   |
| System prompt buffer               | Internal SRAM          | ~16 KB   |
| LLM response stream buffer         | Internal SRAM          | ~32 KB   |
| Remaining available                | Internal SRAM          | ~7.7 MB  |

Large buffers (32 KB+) are allocated from Internal SRAM via `heap_caps_calloc(1, size, MALLOC_CAP_RAM)`.
...
| `heap_info`                    | Show internal + Internal SRAM free bytes     |
