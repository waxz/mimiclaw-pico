...
static void strip_think_tags(char *str) {
  if (!str) return;
  char *start = strstr(str, "<think>");
  while (start) {
    ...
    if (end) {
      ...
      /* Missing closing tag, truncate */
      *start = '\0';
      break;
    }
    start = strstr(str, "<think>");
  }

  /* Strip leading whitespace/newlines */
  int shift = 0;
  while (str[shift] == '\n' || str[shift] == '\r' || str[shift] == ' ') shift++;
  if (shift > 0) {
...
static void append_turn_context_prompt(char *prompt, size_t size,
  ...
  int n = snprintf(prompt + off, size - off,
                   ...
                   "- source_channel: %s\n"
                   "- source_chat_id: %s\n"
                   "- If using cron_add for Telegram in this turn, set "
                   "channel='telegram' and chat_id to source_chat_id.\n"
...
static cJSON *build_tool_results(const llm_response_t *resp,
                                 const mimi_msg_t *msg, char *tool_output,
...
static void agent_loop_task(void *arg) {
  ...
  while (1) {
    ...
    /* Log memory status */
    ESP_LOGI(TAG, "Free PSRAM: %d bytes", (int)esp_get_free_heap_size());
  }
}
...
esp_err_t agent_loop_start(void) {
  ...
  for (size_t i = 0;
    ...
    BaseType_t ret = xTaskCreate(agent_loop_task, "agent_loop", stack_size,
                                 NULL, MIMI_AGENT_PRIO, NULL);
