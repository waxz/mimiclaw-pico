...
static void outbound_dispatch_task(void *arg) {
  ...
  while (1) {
    ...
    if (strcmp(msg.channel, MIMI_CHAN_TELEGRAM) == 0) {
      ...
      if (send_err != ESP_OK) {
        ESP_LOGE(TAG, "Telegram send failed for %s: %s", msg.chat_id,
                 esp_err_to_name(send_err));
      } else {
        ESP_LOGI(TAG, "Telegram send success for %s (%d bytes)", msg.chat_id,
                 (int)strlen(msg.content));
      }
    } else if (strcmp(msg.channel, MIMI_CHAN_FEISHU) == 0) {
      esp_err_t send_err = feishu_send_message(msg.chat_id, msg.content);
      if (send_err != ESP_OK) {
        ESP_LOGE(TAG, "Feishu send failed for %s: %s", msg.chat_id,
                 esp_err_to_name(send_err));
      } else {
        ESP_LOGI(TAG, "Feishu send success for %s (%d bytes)", msg.chat_id,
                 (int)strlen(msg.content));
      }
    } else if (strcmp(msg.channel, MIMI_CHAN_WEBSOCKET) == 0) {
      esp_err_t ws_err = ws_server_send(msg.chat_id, msg.content);
      if (ws_err != ESP_OK) {
        ESP_LOGW(TAG, "WS send failed for %s: %s", msg.chat_id,
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "Heap free:     %d bytes", (int)esp_get_free_heap_size());
  ...
  if (wifi_err == ESP_OK) {
    ...
    if (wifi_manager_wait_connected(30000) == ESP_OK) {
      ...
      /* Outbound dispatch task should start first to avoid dropping early
       * replies. */
      ESP_ERROR_CHECK(
          (xTaskCreate(outbound_dispatch_task, "outbound", MIMI_OUTBOUND_STACK,
                       NULL, MIMI_OUTBOUND_PRIO, NULL) == pdPASS)
              ? ESP_OK
              : ESP_FAIL);
