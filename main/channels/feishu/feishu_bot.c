...
static bool pb_skip_field(const uint8_t *buf, size_t len, size_t *pos,
                          uint8_t wire_type) {
  uint64_t n = 0;
  switch (wire_type) {
  case 0:
...
static bool pb_parse_header_msg(const uint8_t *buf, size_t len,
                                ws_header_t *h) {
  memset(h, 0, sizeof(*h));
  size_t pos = 0;
  while (pos < len) {
    ...
    if (field == 1) {
      size_t n =
          (slen < sizeof(h->key) - 1) ? (size_t)slen : sizeof(h->key) - 1;
      memcpy(h->key, buf + pos, n);
      h->key[n] = '\0';
    } else if (field == 2) {
      size_t n =
          (slen < sizeof(h->value) - 1) ? (size_t)slen : sizeof(h->value) - 1;
  ...
  while (pos < len) {
    ...
    if (field == 1 && wt == 0) {
      ...
      if (f->header_count < 16) {
        pb_parse_header_msg(buf + pos, (size_t)blen,
                            &f->headers[f->header_count++]);
      }
      pos += (size_t)blen;
...
static bool pb_write_varint(uint8_t *buf, size_t cap, size_t *pos,
                            uint64_t value) {
  do {
    if (*pos >= cap)
      return false;
...
static bool pb_write_tag(uint8_t *buf, size_t cap, size_t *pos, uint32_t field,
                         uint8_t wt) {
...
static int ws_send_frame(const ws_frame_t *f, const uint8_t *payload,
                         size_t payload_len, int timeout_ms) {
  uint8_t out[2048];
  size_t pos = 0;
  if (!pb_write_tag(out, sizeof(out), &pos, 1, 0) ||
      !pb_write_varint(out, sizeof(out), &pos, f->seq_id))
    return -1;
  if (!pb_write_tag(out, sizeof(out), &pos, 2, 0) ||
      !pb_write_varint(out, sizeof(out), &pos, f->log_id))
    return -1;
  if (!pb_write_tag(out, sizeof(out), &pos, 3, 0) ||
      !pb_write_varint(out, sizeof(out), &pos, (uint32_t)f->service))
    return -1;
  if (!pb_write_tag(out, sizeof(out), &pos, 4, 0) ||
      !pb_write_varint(out, sizeof(out), &pos, (uint32_t)f->method))
    return -1;

  for (size_t i = 0; i < f->header_count; i++) {
    uint8_t hb[256];
    size_t hlen = 0;
    if (!ws_encode_header(hb, sizeof(hb), &hlen, f->headers[i].key,
                          f->headers[i].value))
      return -1;
    if (!pb_write_bytes(out, sizeof(out), &pos, 5, hb, hlen))
      return -1;
  }
  if (payload && payload_len > 0) {
    if (!pb_write_bytes(out, sizeof(out), &pos, 8, payload, payload_len))
      return -1;
  }
  return esp_websocket_client_send_bin(s_ws_client, (const char *)out, pos,
                                       timeout_ms);
}
...
static char *feishu_api_call(const char *url, const char *method,
...
/* ── WS long connection ────────────────────────────────────── */
static bool parse_query_param(const char *url, const char *key, char *out,
                              size_t out_size) {
...
static esp_err_t feishu_pull_ws_config(void) {
  ...
  if (err != ESP_OK || status != 200) {
    ESP_LOGE(TAG, "WS config request failed: err=%s http=%d",
  ...
  if (ccfg) {
    ...
    if (pi && cJSON_IsNumber(pi))
      s_ws_ping_interval_ms = pi->valueint * 1000;
    if (ri && cJSON_IsNumber(ri))
      s_ws_reconnect_interval_ms = ri->valueint * 1000;
    if (rn && cJSON_IsNumber(rn))
      s_ws_reconnect_nonce_ms = rn->valueint * 1000;
  }
  cJSON_Delete(root);
  ESP_LOGI(TAG, "WS config ready: service_id=%d ping=%dms", s_ws_service_id,
           s_ws_ping_interval_ms);
  return ESP_OK;
}
...
static void feishu_handle_ws_frame(const uint8_t *buf, size_t len) {
  ...
  if (frame.method == 0) {
    if (type && strcmp(type, "pong") == 0 && frame.payload &&
        frame.payload_len > 0) {
      cJSON *cfg =
          cJSON_ParseWithLength((const char *)frame.payload, frame.payload_len);
      if (cfg) {
...
static void feishu_ws_task(void *arg) {
  ...
  while (1) {
    ...
    esp_websocket_client_config_t ws_cfg = {
        ...
        .ping_interval_sec = s_ws_ping_interval_ms / 1000,
        .pingpong_timeout_sec = (s_ws_ping_interval_ms / 1000) + 30,
    ...
    while (s_ws_client) {
      if (s_ws_connected) {
        ...
        if (now - last_ping >= s_ws_ping_interval_ms) {
          ...
          strncpy(ping.headers[0].value, "ping",
                  sizeof(ping.headers[0].value) - 1);
          ws_send_frame(&ping, NULL, 0, 1000);
          last_ping = now;
        }
      } else {
        /* Not connected - either still handshaking or dropped */
        connecting_ticks++;
        /* Break if we've been trying to connect/disconnected for 20 seconds (100 * 200ms) */
        if (connecting_ticks > 100) {
          ESP_LOGW(TAG, "Connection timeout or dropped, restarting WS client");
          break;
        }
      }
 ...
 * "sender_type": "user" }, "message": { "message_id": "om_xxx", "chat_id":
 * "oc_xxx", "chat_type": "p2p" | "group", "message_type": "text", "content":
 * "{\"text\":\"hello\"}"
 *     }
 *   }
 * }
...
static void handle_message_event(cJSON *event) {
  ...
  const char *message_id =
      cJSON_IsString(message_id_j) ? message_id_j->valuestring : "";
  const char *chat_id = chat_id_j->valuestring;
  const char *chat_type =
      cJSON_IsString(chat_type_j) ? chat_type_j->valuestring : "p2p";
  const char *msg_type =
      cJSON_IsString(msg_type_j) ? msg_type_j->valuestring : "text";
  ...
  ESP_LOGI(TAG, "Message from %s in %s(%s): %.60s%s", sender_id, chat_id,
           chat_type, cleaned, strlen(cleaned) > 60 ? "..." : "");
...
/* Webhook mode intentionally disabled: Feishu channel runs in WebSocket mode
 * only. */
...
esp_err_t feishu_bot_init(void) {
  ...
  if (nvs_open(MIMI_NVS_FEISHU, NVS_READONLY, &nvs) == ESP_OK) {
    ...
    if (nvs_get_str(nvs, MIMI_NVS_KEY_FEISHU_APP_ID, tmp_id, &len_id) ==
            ESP_OK &&
        tmp_id[0]) {
      strncpy(s_app_id, tmp_id, sizeof(s_app_id) - 1);
    }
    if (nvs_get_str(nvs, MIMI_NVS_KEY_FEISHU_APP_SECRET, tmp_secret,
                    &len_secret) == ESP_OK &&
        tmp_secret[0]) {
  ...
  if (s_app_id[0] && s_app_secret[0]) {
    ...
    ESP_LOGW(TAG, "No Feishu credentials. Use CLI: set_feishu_creds <APP_ID> "
                  "<APP_SECRET>");
  }
...
esp_err_t feishu_bot_start(void) {
  ...
  BaseType_t ok =
      xTaskCreate(feishu_ws_task, "feishu_ws", MIMI_FEISHU_POLL_STACK, NULL,
                  MIMI_FEISHU_POLL_PRIO, &s_ws_task);
  if (ok != pdPASS) {
    s_ws_task = NULL;
    return ESP_FAIL;
  }
...
esp_err_t feishu_send_message(const char *chat_id, const char *text) {
  ...
  char url[256];
  snprintf(url, sizeof(url), "%s?receive_id_type=%s", FEISHU_SEND_MSG_URL,
           id_type);
      ...
      if (resp) {
        ...
        if (root) {
          ...
          if (code && code->valueint != 0) {
            ...
            ESP_LOGW(TAG, "Send failed: code=%d, msg=%s", code->valueint,
