  ...
  cJSON_ArrayForEach(update, result) {
    ...
    if (msg_id_val >= 0) {
      ...
      if (seen_msg_contains(msg_key)) {
        ESP_LOGW(TAG,
                 "Drop duplicate message update_id=%" PRId64
                 " chat=%s message_id=%d",
    ...
    ESP_LOGI(TAG,
             "Message update_id=%" PRId64
             " message_id=%d from chat %s: %.40s...",
...
static void telegram_poll_task(void *arg) {
  ...
  while (1) {
    ...
    char params[128];
    snprintf(params, sizeof(params), "getUpdates?offset=%" PRId64 "&timeout=%d",
...
esp_err_t telegram_bot_start(void) {
  BaseType_t ret =
      xTaskCreate(telegram_poll_task, "tg_poll", MIMI_TG_POLL_STACK, NULL,
