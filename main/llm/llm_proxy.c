...
{
    if (s_base_url[0]) return s_base_url;
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (nvs_open(MIMI_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        ...
        char base_tmp[128] = {0};
        len = sizeof(base_tmp);
        if (nvs_get_str(nvs, MIMI_NVS_KEY_BASE_URL, base_tmp, &len) == ESP_OK && base_tmp[0]) {
            safe_copy(s_base_url, sizeof(s_base_url), base_tmp);
        }
        nvs_close(nvs);
    }
...
static esp_err_t llm_http_via_proxy(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    strncpy(host, llm_api_host(), sizeof(host)-1);
    strncpy(path, llm_api_path(), sizeof(path)-1);
